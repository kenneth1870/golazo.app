...
function MiniStandingsTable({ group, rows }) {
  ...
  return (
    <table style={{ width: "100%", borderCollapse: "collapse", fontSize: "0.75rem", marginTop: 8 }}>
      ...
      <tbody>
        {rows.map((s, i) => (
          <tr key={i} style={{ borderTop: "1px solid var(--border)", background: i < 2 ? "rgba(238,30,70,.04)" : "transparent" }}>
            <td style={{ padding: "5px 6px", display: "flex", alignItems: "center", gap: 6 }}>
              ...
              {s.team?.flag_url && <img src={s.team.flag_url} alt="" style={{ width: 16, height: 11, objectFit: "cover", borderRadius: 1, flexShrink: 0 }} onError={e => (e.target.style.display = "none")} />}
