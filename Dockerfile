...
# docker build -t golazo_app .
# docker run -d -p 80:80 -e RAILS_MASTER_KEY=<value from config/master.key> --name golazo_app golazo_app
...
# Install base packages (libpq for PostgreSQL, no sqlite3)
...
# Install packages needed to build gems + Node.js (system package, Debian Bookworm ships 18.x)
RUN apt-get update -qq && \
